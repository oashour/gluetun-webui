import { $, setEl, escHtml, showToast } from './util.js';
import * as SS from './server-selector.js';

const MAX_HISTORY = 30;
const VALID_STATES = new Set(['connected', 'paused', 'disconnected', 'unknown']);

let instances    = [];   // [{ id, name, serverSelector, providers? }] from /api/instances
let isPolling    = false;
...
function buildDashboardGroup(inst) {
  ...
  group.innerHTML = `
    ...
    <div class="dashboard-grid">
      ...
      ${inst.serverSelector ? SS.buildServerSelectorCardHTML(inst) : ''}
  ...
  if (inst.serverSelector) {
    SS.initServerSelectorCard(inst, group, { pollAll, scheduleNextPoll });
  }
...
function updatePanel(inst, health) {
  ...
  if (inst.serverSelector && s?.provider?.name) {
    SS.syncFromHealth(inst, s);
  }
...
(async () => {
  try {
    ...
    instances = [{ id: '1', name: 'Gluetun', serverSelector: false }];
  }
  renderAllDashboards();
  await pollAll();
  scheduleNextPoll();
