...
const ENABLE_SERVER_SELECTOR = /^(1|true|yes)$/i.test(process.env.ENABLE_SERVER_SELECTOR || '');
...
// --- Server selector module (feature-gated) ---
// Must be wired before /api/:id/vpn/:action so PUT /vpn/settings registers first.
const serverSelector = ENABLE_SERVER_SELECTOR ? require('./server-selector') : null;
if (serverSelector) {
  serverSelector.parseAllProviders({ instances });
  serverSelector.registerRoutes(app, { resolveInstance, gluetunFetch, buildAuthHeadersFor, vpnActionLimiter });
}
...
app.get('/api/instances', (req, res) => {
  res.json(instances.map(({ id, name }) => ({
    id,
    name,
    serverSelector: ENABLE_SERVER_SELECTOR,
    ...(ENABLE_SERVER_SELECTOR ? { providers: serverSelector.getProvidersForInstance(id) } : {}),
  })));
});
